...
const Details = ( {type,origin,loginType,state, props, handlers, } ) => {
    const { language,panelShow, promotionInfo:{ salesPromotionId,startDate,disableDate, externalAssume, profitsAgainst, salesPromotionName, siteLan,currency,salesPromotionRuleStr, stockType, quantityTotal, productCategory, remark }={} } = state
    ...
    return (
        <div className="activity-entry-info">
            ...
            <div className="tab-name">{language['salespromotion.Promotion_detail']}</div>
            <div className=" info-details">
                <div className="collapse-panel" style={panelShow ? {height: 'auto'} : {height: 54}}>
                    <section className="modules">
                        ...
                        <FormGroup label={language['salespromotion.currency']} value={currency}/>
                        <FormGroup label={language['salespromotion.pay_brear']} value={`${!!externalAssume?`${language['salespromotion.vendor']}${language['salespromotion.bear']}：${externalAssume}%`:''}${(externalAssume==0||externalAssume==100)?'':','}${!!profitsAgainst?`，${language['salespromotion.ctrip']}${language['salespromotion.bear']}：${profitsAgainst}%${language["salespromotion.sentence_of_ten"]}`:''}`}/>
                    </section>
                    <section className="modules">
                        <FormGroup label={language['promotion.list.Activityname']} value={salesPromotionName}/>
                        <FormGroup label={language['ruletemplate.locale']} value={siteLan}/>
