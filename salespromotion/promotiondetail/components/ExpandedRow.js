...
export default class ExpandedRow extends React.Component {
    ...
    render() {
        ...
        const content = ({ doingSize, unStartSize }) => <div>
            <div>{language["promotion.search.inprogress"]}：<span style={{ color: '#f81d22' }}>{doingSize}</span></div>
            <div>{language["promotion.search.Notyetbegun"]}：<span style={{ color: '#f81d22' }}>{unStartSize}</span></div>
        </div>
