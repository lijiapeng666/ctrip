    ...
    return (
        <div className="promotion-modal">
            ...
            <div style={{ margin: "32px 0 8px" }}>
                ...
                (<span>{language['promotion.search.inprogress']}：<span style={{ color: '#f81d22' }}>{doingSize}</span></span>，<span>{language['promotion.search.Notyetbegun']}：<span style={{ color: '#f81d22' }}>{unStartSize}</span></span>)
            </div>
            ...
            >
                ...
                <Column
                    ...
                    render={(text, { profitsAgainst, externalAssume }) => (
                        <span>
                            {`${language['salespromotion.vendor']}${externalAssume}%`}{profitsAgainst > 0 ? `，${language['salespromotion.ctrip']}${profitsAgainst}%${language["salespromotion.sentence_of_ten"]}` : ''}
                        </span>
                    )}
                />
                <Column
                ...
                <Column
                    title={language["promotion.list.ActivityCreator"]}
                    dataIndex="sponser"
                    width={105}
                    render={(text) => {
                        let name = text
